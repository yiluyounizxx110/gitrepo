var pieChart = {  
    ...
    render : function() {  
        ...
        if(sum<100){
        	this.others = {name:"", value:100 -sum, color:"RGBA(228,0,126,1)"}
        }
        ...
        ctx.save();  
        var deltaArc = -Math.PI/2;  
        for(var i=0; i<nums; i++) {  
            this.renderPie(ctx, this.series[i], this.series[i].value/100, deltaArc,true);  
            deltaArc += 2*Math.PI * this.series[i].value/100;  
        } 
        if(this.others != null){
        	this.renderPie(ctx, this.others, this.others.value/100, deltaArc,false);  
//          deltaArc += 2*Math.PI * this.others.value/100;  
        }
    ...
    renderPie : function(ctx, obj, precent, deltaArc,showtip) {  
...
//          var size = this.text.content.length;  
            var tipStr = (precent * 100).toFixed(0) + "%";  
